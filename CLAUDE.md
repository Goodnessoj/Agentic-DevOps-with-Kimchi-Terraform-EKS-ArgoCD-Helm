...
- **Provider:** AWS provider ~> 5.0, region us-east-2
- **ECR:** Uses `aws_ecr_repository` in us-east-2 with lifecycle policies, scan-on-push, and configurable tag immutability
...
- ECR repos: `{account}.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/{service-name}`
...
- **ECR login:** `aws ecr get-login-password --region us-east-2` (same region as infrastructure)
