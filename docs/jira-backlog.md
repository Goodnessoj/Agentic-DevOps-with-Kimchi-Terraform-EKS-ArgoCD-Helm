...
**Priority:** P0
**Description:** Create ECR private repositories for all 8 microservices with lifecycle policies, scan-on-push, and configurable tag immutability (MUTABLE dev, IMMUTABLE prod). Images stored at `{account}.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/{service}:{tag}`. Cost: ~$1/month beyond 500 MB free tier.
**Blocked by:** E-1
**Blocks:** E-10, E-17
...
- [ ] Uses `aws ecr get-login-password --region us-east-2` and pipes to `docker login {account}.dkr.ecr.us-east-2.amazonaws.com`
...
- [ ] Image: `{account}.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/config-server:<TAG>` (placeholder)
...
- [ ] Push to ECR: `{account}.dkr.ecr.us-east-2.amazonaws.com/petclinic-dev/{service}:{sha}`
...
- [ ] ECR login successful: `aws ecr get-login-password --region us-east-2 | docker login --username AWS --password-stdin {account}.dkr.ecr.us-east-2.amazonaws.com`
- [ ] All 8 images tagged with initial version (e.g., `v1.0.0` or commit SHA)
- [ ] All 8 images pushed to ECR (`{account}.dkr.ecr.us-east-2.amazonaws.com/petclinic-dev/{service}:{tag}`)
...
- [ ] Compliance: us-east-2 (US region) — review data residency requirements if applicable
...
- [ ] Each file specifies: image repo (`{account}.dkr.ecr.us-east-2.amazonaws.com/petclinic-{env}/{service}`), image tag, container port, service port
